...
EAS Build uploads source maps as part of the native build, so a build workflow needs nothing extra. For updates, use the [`eas/posthog_upload_sourcemaps`](/eas/workflows/syntax/#easposthog_upload_sourcemaps) function and publish with `--skip-bundler`, so the update matches the exact bundle you uploaded source maps for.

<BoxLink
  title="Upload source maps for error tracking"
  description="A complete workflow that exports the bundle, uploads its source maps, and publishes the exact bundle it uploaded."
